/* Waffle clone — pure board logic.
 *
 * Shared by the browser game (js/game.js) and the Node test suite
 * (tools/test_core.js).  Nothing here touches the DOM.
 *
...
(function (root, factory) {
  ...
  function solutionLetters(across, down) {
   ...
   */
  function gradeWord(solutionWord, guessWord) {
   ...
   *
   * `letters` is the current arrangement in cell order.  A tile is green when
   * it matches the solution at that cell, otherwise it is graded against its
   * across word (or its down word, for tiles on the outer edge).
   */
  function gradeBoard(letters, across, down) {
  ...
  function isSolved(letters, solution) {
    return letters.every((letter, index) => letter === solution[index]);
  }

  return { SIZE, CELLS, cellIndex, solutionLetters, gradeWord, gradeBoard, isSolved };
});
